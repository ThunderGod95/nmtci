...
const SwipeManager = {
    ...
    log(...args) {
        if (this.debug) console.log("[SwipeManager]", ...args);
    },

    init() {
        this.log("Initializing SwipeManager");
        this.handlePointerDown = this.handlePointerDown.bind(this);
        this.handlePointerUp = this.handlePointerUp.bind(this);
        this.handlePointerCancel = this.resetSwipe.bind(this);
    ...
    handlePointerDown(e) {
        ...
        if (e.pointerType !== "touch" && e.pointerType !== "pen") {
            this.log("Ignored: Wrong pointer type", e.pointerType);
            return;
        }
        if (
            e.target instanceof Element &&
            e.target.closest(".settings-menu, .audio-menu, input, button, select, a")
        )
            return;
    ...
    handlePointerUp(e) {
        if (this.touchStartX === null) {
            this.log("Pointer Up ignored: No start coordinates");
            return;
        }
        ...
        if (duration > this.maxSwipeTime) {
            this.log(`Swipe failed: Too slow (${duration}ms > ${this.maxSwipeTime}ms)`);
            return;
        }

        this.evaluateSwipe(endX, endY);
    },

    evaluateSwipe(endX, endY) {
        const xDiff = endX - this.touchStartX;
        const yDiff = endY - this.touchStartY;
        const absX = Math.abs(xDiff);
        const absY = Math.abs(yDiff);

        this.log("Evaluating Swipe:", { xDiff, yDiff, absX, absY });

        if (absX < this.minSwipeDistance) {
            this.log(`Swipe failed: Distance too short (${absX}px < ${this.minSwipeDistance}px)`);
            return;
        }

        if (absY > absX * this.maxVerticalRatio) {
            this.log(
                `Swipe failed: Too much vertical drift (Ratio: ${(absY / absX).toFixed(2)} > ${this.maxVerticalRatio})`,
            );
            return;
        }

        if (window.getSelection().toString().length > 0) {
            this.log("Swipe failed: Text is selected");
            return;
        }

        if (xDiff > 0) {
            this.log("Swipe Direction: RIGHT (Previous)");
            if (typeof NavigationManager !== "undefined" && NavigationManager.prevUrl) {
                this.log("Navigating to:", NavigationManager.prevUrl);
                window.location.href = NavigationManager.prevUrl;
            } else {
                this.log("Navigation failed: NavigationManager.prevUrl not found");
            }
        } else {
            this.log("Swipe Direction: LEFT (Next)");
            if (typeof NavigationManager !== "undefined" && NavigationManager.nextUrl) {
                this.log("Navigating to:", NavigationManager.nextUrl);
                window.location.href = NavigationManager.nextUrl;
            } else {
                this.log("Navigation failed: NavigationManager.nextUrl not found");
            }
        }
    },
    ...
    resetSwipe() {
        if (this.touchStartX !== null) {
            this.log("Resetting swipe state");
        }
